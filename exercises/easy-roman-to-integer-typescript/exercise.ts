...
export function romanToInt(s: string): number {
    ...
    let sum = 0;
    let previousInt: number | undefined = undefined;

    for(let  i = 0;i < s.length; i++) {
      let currentRoman = s[i];

      let currentInt = map.get(currentRoman.toUpperCase()) || 0;

      if(previousInt && previousInt < currentInt) {
        // added the previous already, so remove it twice
        sum += currentInt - 2*previousInt;
      } else  {
        sum += currentInt;
      }
